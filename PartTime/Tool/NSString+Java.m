...
/** Java-like method. Compares two strings lexicographically, ignoring case
 * differences. */
- (NSInteger)compareToIgnoreCase:(NSString *)str {
  return [self compare:str options:NSCaseInsensitiveSearch];
}

/** Java-like method. Returns true if and only if this string contains the
...
- (NSInteger)indexOfString:(NSString *)str fromIndex:(NSInteger)index {
  ...
  NSRange range =
      [self rangeOfString:str options:NSLiteralSearch range:fromRange];
  if (range.location == NSNotFound) {
    return JavaNotFound;
  }
  return range.location;
...
- (NSInteger)lastIndexOfString:(NSString *)str fromIndex:(NSInteger)index {
  ...
  NSRange range =
      [self rangeOfString:str options:NSBackwardsSearch range:fromRange];
  if (range.location == NSNotFound) {
    return JavaNotFound;
  }
  return range.location;
...
- (NSString *)replaceAll:(NSString *)origin with:(NSString *)replacement {
  return
      [self stringByReplacingOccurrencesOfString:origin withString:replacement];
}
...
+ (BOOL)isEmpty:(NSString *)text {
  return text == nil || [@"" isEqualToString:text] || text.length <= 0;
}
